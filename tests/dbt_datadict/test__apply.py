...
def test__dictionary_can_be_updated(
    ...
    file = tmp_path / "dict.yml"
    utils.YAML.dump(original, file)
    datadict = apply.DataDict(str(file))
    ...
    assert datadict.dictionary_yml == expected
...
def test__dictionary_can_be_parsed_without_aliases(
    ...
    file = tmp_path / "dict.yml"
    utils.YAML.dump(dictionary, file)
    datadict = apply.DataDict(str(file))

    assert datadict.dictionary_items == ["field1", "field2"]
...
def test__dictionary_can_be_parsed_with_aliases(
    ...
    assert datadict.dictionary_items == ["field1", "f1", "alias1", "field2"]


def test__dictionary_items_can_be_inserted():
    """
